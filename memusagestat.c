...
{
  { "output", 'o', "FILE",   0, "Name output file", 0},
  { "string", 's', "STRING", 0, "Title string used in output graphic", 0 },
  { "time",   't', NULL,     0, "Generate output linear to time (default is linear to number of function calls)", 0 },
  { "total",  'T', NULL,     0, "Also draw graph for total memory consumption", 0 },
  { "x-size", 'x', "VALUE",  0, "Make output graphic VALUE pixels wide", 0 },
  { "y-size", 'y', "VALUE",  0, "Make output graphic VALUE pixels high", 0 },
  { NULL, 0, NULL, 0, NULL, 0 }
};

/* Short description of program.  */
static const char doc[] = "Generate graphic from memory profiling data";

/* Strings for arguments in help texts.  */
static const char args_doc[] = "DATAFILE [OUTFILE]";
...
main (int argc, char *argv[])
{
  ...
  const char *inname;
  gdImagePtr im_out;
  int grey __attribute__((unused)); /* used as background, not really used */
  int blue, red, green, yellow, black;
  int fd;
  struct stat st;
  size_t maxsize_heap;
  ...
  if (remaining >= argc || remaining + 2 < argc)
    {
      argp_help (&argp, stdout, ARGP_HELP_SEE | ARGP_HELP_EXIT_ERR, program_invocation_short_name);
      exit (1);
    }
  ...
  if ((st.st_size % sizeof (struct entry)) != 0
      ...
      || (size_t)st.st_size < 2 * sizeof (struct entry))
    {
      close (fd);
